...
const radioButtons = document.querySelectorAll('.radiobox__button_input');
...
function changeCards(evt) {
  ...
  equalizeCheckboxes(evt);
}

// Приведение в соответствие селектора и радиокнопок выбора типа велосипедов
function equalizeCheckboxes(evt) {
  if (evt.target.classList.contains('selector')) {
    ...
    cardSelector.value = evt.target.value;
  }
}

export default function initBikeSelector() {
  radioButtons.forEach((button) => {
